...
The current built-in Raw example contains six maize samples: three B73 replicates and three EA2024 replicates. Its columns include `sample_id`, `mzml_file`, genotype, treatment, group, condition, replicate, batch, tissue, organism, accession, and the PRIDE source URL.

The built-in filenames are a template. Replace `mzml_file` values with the actual filenames present in your selected directory when your downloaded files use different names.
